...
export default async function AdminContactsPage(props: {
  ...
  if (params.memberId) filterQuery.set("memberId", params.memberId);
  if (params.category) filterQuery.set("category", params.category);
  if (params.visibility) filterQuery.set("visibility", params.visibility);
  const filterQs = filterQuery.toString() ? `?${filterQuery.toString()}` : "";
      ...
      <div className="max-w-5xl mx-auto">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-neutral-800 pb-4 mb-6">
          <div>
            ...
            <p className="text-neutral-500 text-sm mt-0.5">Support view of member contact databases.</p>
          </div>
        ...
        <p className="text-neutral-500 text-sm mb-3">{contacts.length} contact{contacts.length === 1 ? "" : "s"}{contacts.length >= 200 ? " (showing first 200)" : ""}</p>

        {contacts.length === 0 && !dbError ? (
          <p className="text-neutral-500 text-sm">No contacts match. Members can add them in the portal, or use Import contacts on a member row.</p>
        ) : (
                ...
                {contacts.map((c) => (
                  <tr key={c.id} className="border-b border-neutral-800">
                    <td className="py-2 pr-3">
                      <Link href={`/admin/contacts/${c.id}${filterQs ? `?return=${encodeURIComponent("/admin/contacts" + filterQs)}` : ""}`} className="text-emerald-400 hover:underline font-medium">
                        {contactDisplayName(c)}
                      </Link>
                      {c.jobTitle && <p className="text-neutral-500 text-xs">{c.jobTitle}</p>}
                    </td>
                    <td className="py-2 pr-3 text-neutral-400">
                      <Link href={`/admin/contacts?memberId=${c.member.id}`} className="hover:text-white">
                        {c.member.email}
                      </Link>
                    </td>
                    <td className="py-2 pr-3 text-neutral-400">
